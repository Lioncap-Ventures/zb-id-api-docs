...
function CodePanel({
  ...
  // Accept multiple children safely. Prefer extracting metadata from the
  // first valid React element child if present, otherwise fall back to the
  // first child in the array.
  const childArray = Children.toArray(children)
  const firstChild = childArray.find((c) => isValidElement(c)) ?? childArray[0]

  if (isValidElement(firstChild)) {
    const element = firstChild as React.ReactElement<Record<string, unknown>>
