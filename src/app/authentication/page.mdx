...
export const sections = [
  ...
  { title: 'Password reset', id: 'password-reset' },
  { title: 'Deactivating an account', id: 'deactivate-account' },
...
<Note>
  These docs are interactive. The panels below run live against the ZB ID staging sandbox. Register a throwaway account, then sign in, and the access token carries across the token-scoped panels on this and other pages.
</Note>
...
<Row>
  ...
  <Col sticky>
    ...
    response = requests.post(
        'https://id.zb.co.zw/auth/register',
...
<TryIt
  method="POST"
  path="/auth/register"
  title="Register a throwaway test account"
  description="Runs live against staging. Use a made-up email like you@example.com; the response includes an access token that the token-scoped panels on other pages will reuse."
  seedsToken
  defaultBody={{
    email: 'tryit-user@example.com',
...
<TryIt
  method="POST"
  path="/auth/login"
  title="Sign in and seed a token"
  description="Sign in with the account you registered above. A successful login captures the access token so the token-scoped panels (current user, sessions, introspection) can chain from it automatically."
  seedsToken
  defaultBody={{
    email: 'tryit-user@example.com',
    password: 'TestPass123!',
...
<Row>
  ...
  <Col sticky>
    ...
    response = requests.post(
        'https://id.zb.co.zw/auth/password/change',
        headers={
...
## Password reset {{ id: 'password-reset', tag: 'POST', label: '/auth/password/forgot' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Step 1: request link" tag="POST" label="/auth/password/forgot">
    ...
    ```json {{ title: 'Response (always 200)' }}
    {
      "message": "If an account exists, a reset link has been sent."
    }
    ```

    <CodeGroup title="Step 2: reset password" tag="POST" label="/auth/password/reset">
...
## Deactivating an account {{ id: 'deactivate-account', tag: 'POST', label: '/auth/deactivate' }}
...
<Row>
  ...
  <Col sticky>
    ...
    <CodeGroup title="Request" tag="POST" label="/auth/deactivate">
...
<Row>
  ...
  <Col sticky>
    ...
    response = requests.post(
        'https://id.zb.co.zw/auth/token/refresh',
        headers={'Content-Type': 'application/json'},
...
<Row>
  <Col>
    ...
    | `iss` | string | Token issuer (the ZB ID issuer identifier, `https://id.zb.co.zw`) |
