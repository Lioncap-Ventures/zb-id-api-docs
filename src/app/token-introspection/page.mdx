...
<Row>
  <Col>
    ...
    <Note>
      User tokens do **not** carry a `tokenType` value; distinguish them by the absence of `tokenType` (or by the presence of `aud`, `roles`, and `kycTier`). Client tokens have `tokenType: "client"` and a `clientId`.
    </Note>
  ...
  <Col sticky>
    ...
    response = requests.post(
        'https://id.zb.co.zw/oauth/introspect',
    ...
    ```json {{ title: 'Response (active user token)' }}
    {
      "active": true,
...
<TryIt
  ...
  title="Introspect your chained token"
  description="Sign in on the Authentication page first. This panel drops that access token into the request body and validates it. With no token it returns active:false; with a valid one it returns the full claims set."
  useToken
  tokenBodyField="token"
  defaultBody={{ token: 'paste-a-token-here' }}
/>
...
<Row>
  ...
  <Col sticky>
    ...
    <Note>
      **Security tip:** Never assume a token is a user token just because it has scopes. A user token has no `tokenType`; a client token has `tokenType: "client"`. A client token with `profile:read` should not be used to fetch a specific user's profile without additional authorization logic.
    </Note>
