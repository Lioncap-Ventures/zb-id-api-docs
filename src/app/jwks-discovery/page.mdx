...
<Row>
  <Col>
    ...
    The URLs in the discovery document are derived from the issuer identifier (`https://id.zbfintech.com`), which is also the value carried in every token's `iss` claim. When you make requests, send them to the documented base URL (`https://zbid-api.lioncapventures.com`); use the issuer identifier only to match the `iss` claim during token verification.

    ### Response fields

    <Properties>
      <Property name="issuer" type="string">
        The issuer identifier of the ZB ID authorization server (`https://id.zbfintech.com`). Tokens issued by ZB ID carry this value in their `iss` claim, so verify the `iss` claim against it.
      </Property>
  ...
  <Col sticky>
    ...
    ```json {{ title: 'Response' }}
    {
      "issuer": "https://id.zbfintech.com",
      "jwks_uri": "https://id.zbfintech.com/.well-known/jwks.json",
      "token_endpoint": "https://id.zbfintech.com/oauth/token",
      "introspection_endpoint": "https://id.zbfintech.com/oauth/introspect",
...
<Row>
  ...
  <Col sticky>
    ...
    ZB_ID_ISSUER = 'https://id.zbfintech.com'
