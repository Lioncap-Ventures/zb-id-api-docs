...
<Row>
  <Col>
    ...
    The URLs in the discovery document all sit on the production base URL (`https://zbid-api.lioncapventures.com`), which is also the issuer identifier carried in every token's `iss` claim. The staging environment issues tokens with `iss` `https://zbid-api-staging.lioncapventures.com`.

    ### Response fields

    <Properties>
      <Property name="issuer" type="string">
        The issuer identifier of the ZB ID authorization server (`https://zbid-api.lioncapventures.com`). Tokens issued by ZB ID carry this value in their `iss` claim, so verify the `iss` claim against it.
      </Property>
